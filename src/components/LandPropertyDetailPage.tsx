...
import {
  ArrowLeft, MapPin, CheckCircle2, Phone, Calendar, MessageCircle,
  ChevronLeft, ChevronRight, School, Briefcase, Train, Plane, MapPinned,
  X, Maximize, Eye, Share2, Navigation, Clock, TrendingUp, Sparkles,
...
export default function LandPropertyDetailPage({ slug, onBack, onBookVisit }: LandPropertyDetailPageProps) {
  ...
  const phoneDigits = property.contactPhone.replace(/\s/g, '');
  const whatsappDigits = property.contactWhatsapp.replace(/\D/g, '');
  const shareUrl = typeof window !== 'undefined' ? window.location.href : `https://www.meridian4.com/property/${property.slug}`;
  const shareText = `Check out this property: ${property.title} — ${property.address.line2}, ${property.address.city}`;

  return (
    <div className="bg-ivory min-h-screen" style={{ paddingTop: '180px' }}>
      {/* ===== Sticky action + section nav bar ===== */}
      <div
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href={`tel:${phoneDigits}`} className="w-9 h-9 rounded-full glass flex items-center justify-center text-navy hover:text-gold-dark transition-colors" aria-label="Call">
              <Phone size={15} />
            </a>
            <a href={`https://wa.me/${whatsappDigits}`} className="w-9 h-9 rounded-full glass flex items-center justify-center text-navy hover:text-gold-dark transition-colors" aria-label="WhatsApp">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="mt-8 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 flex items-center gap-2">
              <Trees size={15} className="text-gold-dark" /> {property.propertyType}
            </p>
          </div>
          <div className="flex flex-col items-start lg:items-end gap-2">
            ...
            <a
              ...
              className="font-display text-3xl lg:text-4xl text-gold-dark hover:text-gold transition-colors flex items-center gap-2"
            >
              <Phone size={24} /> Contact for Price
            </a>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <a href={`tel:${phoneDigits}`} className="btn-navy flex items-center gap-2">
            <Phone size={18} /> Call Now
          </a>
          <a href={`https://wa.me/${whatsappDigits}`} className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border border-gold text-gold-dark font-semibold text-sm hover:bg-gold hover:text-white transition-all">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="space-y-8">
          ...
          <LuxuryCard id="location" title="Location" icon={MapPinned}>
            <div className="rounded-2xl overflow-hidden h-80 border border-gray-100">
              <iframe
                ...
                src={`https://www.google.com/maps?q=${encodeURIComponent(property.location.mapQuery)}&output=embed`}
              />
            </div>
            <div className="mt-4 p-4 rounded-xl bg-ivory">
              <p className="text-sm font-semibold text-navy mb-1">{property.address.line1}</p>
              <p className="text-sm text-gray-600">{property.address.line2}</p>
              <p className="text-sm text-gray-600">{property.address.city}</p>
              <p className="text-sm text-gray-600">{property.address.state}, {property.address.country}</p>
            </div>
          </LuxuryCard>
          ...
          <LuxuryCard id="contact" title="Contact Information" icon={Phone}>
            <div className="grid sm:grid-cols-2 gap-4">
              <ContactRow icon={Building2} label="Company" value={property.contact.company} />
              <ContactRow icon={Phone} label="Phone" value={property.contact.phone} href={`tel:${phoneDigits}`} />
              <ContactRow icon={MessageCircle} label="WhatsApp" value={property.contact.whatsapp} href={`https://wa.me/${whatsappDigits}`} />
              <ContactRow icon={Mail} label="Email" value={property.contact.email} href={`mailto:${property.contact.email}`} />
            ...
            <div className="flex flex-wrap gap-3 items-center">
              <button
                onClick={() => {
                  ...
                  const msg = `Hello MERIDIAN4, I'd like to schedule a site visit for "${property.title}".%0AName: ${visitorName}%0APhone: ${visitorPhone}%0APreferred slot: ${slot.day} at ${slot.time}%0AProperty: ${property.address.line2}, ${property.address.city}`;
                  setScheduleSubmitted(true);
                  window.open(`https://wa.me/${whatsappDigits}?text=${encodeURIComponent(msg).replace(/%2520/g, '%20')}`, '_blank');
                }}
              ...
              <a href={`https://wa.me/${whatsappDigits}`} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gold text-gold-dark font-semibold text-sm hover:bg-gold hover:text-white transition-all">
                <MessageCircle size={18} /> WhatsApp
              </a>
            </div>
...
function NearbyCard({ place }: { place: NearbyPlace }) {
  const Icon = place.category === 'Railway'
    ? Train
